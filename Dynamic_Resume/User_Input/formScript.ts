...
localStorage.setItem('form_jobTitleExp', jobTitleExp.value);
localStorage.setItem('form_startYear', startYear.value);
localStorage.setItem('form_endYear', endYear.value);
localStorage.setItem('form_profilePic', profilePic.value);
...
if(profilePic.files && profilePic.files[0]){
    ...
    // const reader = new FileReader();
    const Base64Imaga = reader.result as string;
    localStorage.setItem("form_profilePic", Base64Imaga);
    window.location.href="../User_Input/Preview_Resume/DynamicResume.html";
    };
...
console.log(localStorage.getItem("profilePic.files"));
})
